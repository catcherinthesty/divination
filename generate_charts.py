#!/usr/bin/env python3
"""
Generate natal charts for Bristol (bakl_) and Aria (arh_), plus a family relationship chart.
Reads MCP API responses and produces prefixed SVG + HTML files.
"""
...
BRISTOL_RESPONSE = Path("/home/jheinsen/.qwen/tmp/c327bffecf7fcf7080977cf99f9a3601ea181853f0f9831a8e4211837a6a1cf8/tool-results/X10YKNRbmr7BCnockQDBp2QNEu5kM1zd.txt")
ARIA_RESPONSE = Path("/home/jheinsen/.qwen/tmp/c327bffecf7fcf7080977cf99f9a3601ea181853f0f9831a8e4211837a6a1cf8/tool-results/rnbACkDOSEkydOOiMJcW3mfajzEBn7c5.txt")
...
def load_chart(response_path):
    with open(response_path) as f:
        data = json.load(f)
    return data["chart_data"], data["chart"]


def build_planet_table(planets):
    ...
    for pname in planet_order:
        ...
        house = p.get("house", "")
        house_num = str(HOUSE_ORDINALS.get(house.split("_")[0].lower(), "?")) if house else "?"
        ...
        rows.append(
            ...
            f"<td>{house_num}</td><td>{retro}</td><td>{element} {quality}</td></tr>"
        )
    return rows


def build_house_table(sub):
    ...
    for hkey in house_keys:
        ...
        if h:
            ...
            rows.append(f"<tr><td>{emoji} H{num}</td><td>{sign}{deg:.1f}°</td></tr>")
    return rows


def build_aspects(chart_data):
    rows = []
...
def build_interpretation(planets, chart_data):
    ...
    interpretations.append(
        ...
        f'In the {sun_h}th house, self-expression finds its outlet through get_house_meaning({sun_h}).</p>'
    )
    ...
    interpretations.append(
        ...
        f'Moon in the {moon_h}th house suggests emotional security through get_house_meaning({moon_h}).</p>'
    )
...
def generate_html(name, prefix, chart_data, svg_content):
    ...
    planet_rows = build_planet_table(planets)
    house_rows = build_house_table(sub)
    aspect_rows = build_aspects(chart_data)
    ...
    svg_filename = f"{prefix}chart.svg"
...
<div class="container">
  ...
  <div class="svg-wrap">
    <img src="{svg_filename}" alt="Natal Chart">
  </div>
  ...
  <div class="grid">
    ...
    svg_path = PROJECT / f"{prefix}chart.svg"
    with open(svg_path, "w") as f:
        f.write(svg_content)
    print(f"Saved SVG: {svg_path}")

    html_path = PROJECT / f"{prefix}chart.html"
...
def main():
    # --- Bristol (bakl_) ---
    print("=== Generating Bristol Ann Klok-Loomis chart ===")
    cd_b, svg_b = load_chart(BRISTOL_RESPONSE)
    generate_html("Bristol Ann Klok-Loomis", "bakl_", cd_b, svg_b)

    # --- Aria (arh_) ---
    print("\n=== Generating Aria Rose Heinsen chart ===")
    cd_a, svg_a = load_chart(ARIA_RESPONSE)
    generate_html("Aria Rose Heinsen", "arh_", cd_a, svg_a)
